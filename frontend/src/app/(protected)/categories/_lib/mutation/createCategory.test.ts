    ...
    it('カテゴリを作成できる', async () => {
      ...
      describe('エラー時', () => {
        it('404エラー時に適切なエラーメッセージをスローする', async () => {
          ...
          await expect(createCategory(mockCategory)).rejects.toThrow(CATEGORIES_ERROR_MESSAGES.NOT_FOUND);
        });
        ...
        it('500エラー時にデフォルトエラーメッセージをスローする', async () => {
          ...
          await expect(createCategory(mockCategory)).rejects.toThrow(CATEGORIES_ERROR_MESSAGES.UNKNOWN_ERROR);
        });
        ...
        it('ネットワークエラー時にエラーをスローする', async () => {
          ...
          await expect(createCategory(mockCategory)).rejects.toThrow(CATEGORIES_ERROR_MESSAGES.NETWORK_ERROR);
        });
      });
